...
Inspector.showIfNFR = function (part) {
  if (part instanceof go.Node) {
    if (part.data.isNFR === 'No') { return true }
  }
  return false
}
Inspector.showIfOperationGoal = function (part) {
  if(part instanceof go.Node) {
    if (part.data.Type === '<<可操作性目标>>') { return true }
  }
  return false
}
Inspector.showIfRole = function (part) {
  if(part instanceof go.Node) {
    if (part.data.Type === '<<资源>>' || part.data.Type === '<<Resource>>'
    ) { return true }
  }
  return false
}
...
//判断是否为域
Inspector.showIfNodeDomain=function(part){
  if(part instanceof go.Node){
    if( part.data.Type==='<< 节点域 >>'){return true}
  }
  return false
}
Inspector.showIfBPMN=function(part){
  if(part instanceof go.Node){
    if(part.data.Type==='<<流程>>'){return true}
  }
  return false
}
Inspector.showIfActivity=function(part){
  if(part instanceof go.Node){
    if(part.data.category==='activity'){return true}
  }
  return false
}
Inspector.showIfGoToBPMN=function(part){
  if(part instanceof go.Node){
    if(part.data.Type==='<<价值活动>>'||part.data.Type==='<<节点域>>'){return true}
  }
  return false
}

Inspector.showIfFuncDomain=function(part){
  if(part instanceof go.Node){
    if(part.data.Type==='<<功能域>>'){return true}
  }
  return false
}

Inspector.showIfMission=function(part){
  if(part instanceof go.Node){
    if(part.data.key=== 131){return true}
  }
  return false
}
...
Inspector.prototype.inspectObject = function (object) {
  ...
  if (!inspectedObjects || !this.multipleSelection) {
    ...
    if (this.includesOwnProperties) {
      for (var k in data) {
        if (k === '__gohashid') continue // skip internal GoJS hash
        // property
...
Inspector.prototype.buildPropertyRow = function (propertyName, propertyValue) {
  ...
  // 获取自定义名称，如果存在的话
  var displayName = this.declaredProperties[propertyName] && this.declaredProperties[propertyName].name ? this.declaredProperties[propertyName].name : propertyName;

  var td1 = document.createElement('td')
  //td1.textContent = propertyName
  td1.textContent = displayName; // 使用自定义名称或默认属性名称
  tr.appendChild(td1)
