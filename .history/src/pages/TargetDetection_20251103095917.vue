    ...
    <b-row class="justify-content-center pt-3 pb-5 main-content-row"> 
      ...
      <b-col cols="3" class="text-center p-2">
        <div class="data-selection-box">
          ...
          <b-form-file
            v-model="uploadedFile" :state="Boolean(uploadedFile)" placeholder="选择视频..."
          ...
          <div class="selected-list p-3">
            ...
            <ul class="list-unstyled text-left video-list custom-scroll">
              <li 
                ...
                :class="{'active-selection': selectedFile.type === 'upload'}"
                @click="selectVideo({ type: 'upload', name: uploadedFile.name, url: originalVideoURL })"
              >
              ...
              <li 
                v-for="(video, index) in serverVideos" 
                :key="index" 
                :class="{'text-secondary': !selectedFile, 'active-selection': selectedFile.type === 'server' && selectedFile.index === index}"
                @click="selectVideo({ type: 'server', index: index, name: video.name, url: video.url })"
              >
                <b-icon icon="play-circle-fill" variant="dark"></b-icon>
                {{ video.name }}
              </li>
            </ul>
            <p class="text-secondary mt-2 small-text">(当前列表显示 {{ (uploadedFile ? 1 : 0) + serverVideos.length }} 个文件)</p>
          </div>

          <div class="start-button-area pt-3">
            <b-button @click="startDetection" variant="primary" :disabled="!selectedFile.url || isLoading" class="start-btn-large">
      ...
      <b-col cols="4" class="text-center p-2">
        <div class="video-result-container">
          <div class="box-label">原视频 <span class="small-text">({{ selectedFile.name || '此处是视频' }})</span></div>
          <div class="video-placeholder-alt">
            <video v-if="selectedFile.url" :src="selectedFile.url" controls class="image-display" :key="'orig'+selectedFile.url"></video>
...
export default {
  ...
  data() {
    return {
      ...
      uploadedFile: null, // 用于文件上传的 v-model，即用户实际上传的文件
      originalVideoURL: null, // 上传文件在客户端的预览 URL
      selectedFile: { // 当前选中用于检测的文件信息
        type: null, // 'upload' 或 'server'
        name: null,
        url: null, // 视频播放的实际 URL
        index: null, // 仅对 'server' 类型有用
        fileObject: null, // 仅对 'upload' 类型有用，存储 uploadedFile 对象
      },
      ...
      // 示例服务器视频列表 (需要替换为真实的视频链接)
      serverVideos: Array.from({ length: 100 }, (_, i) => ({
        name: `视频${i + 1}.MP4`,
        // **重要**: 这里的 URL 需要是后端能访问到的实际视频链接
    ...
    uploadedFile(newFile) {
      ...
      if (newFile) {
        ...
        this.selectVideo({ 
          type: 'upload', 
          name: newFile.name, 
          url: this.originalVideoURL,
          fileObject: newFile,
        });
      } else {
        // 如果文件被清除，也清除 URL
        this.originalVideoURL = null;
        // 如果当前选中文件是被清除的文件，则清空选中状态
  ...
  methods: {
    ...
    selectVideo(videoInfo) {
      this.selectedFile.type = videoInfo.type;
      this.selectedFile.name = videoInfo.name;
      this.selectedFile.url = videoInfo.url;
      this.selectedFile.index = videoInfo.index; // 可能是 null
      this.selectedFile.fileObject = videoInfo.fileObject || null; // 可能是 null (只有上传文件有 fileObject)
    ...
    async startDetection() {
      ...
      try {
        // **重要：这里需要根据 selectedFile.type 来决定如何向后端发送数据**
        
        // 如果是上传文件，需要通过 FormData 传输文件对象
        if (this.selectedFile.type === 'upload' && this.selectedFile.fileObject) {
            // 这里应该调用后端上传/分析接口，使用 this.selectedFile.fileObject
            // ... 实际的 axios 上传代码 ...
             await new Promise(resolve => setTimeout(resolve, 3000));
        } 
        // 如果是服务器视频，可能只需要发送其 URL 或名称
        else if (this.selectedFile.type === 'server') {
            // 这里应该调用后端分析接口，发送 this.selectedFile.url 或 name
            // ... 实际的 axios 分析代码 (例如 POST /api/analyze?url=...) ...
             await new Promise(resolve => setTimeout(resolve, 3000)); 
        } else {
            throw new Error("选中文件信息不完整或不正确。");
        }
        ...
        // 模拟成功后更新结果
        // this.processedImageURL = 'path/to/result_image.jpg'; 
...
<style lang="scss" scoped>
  ...
  .video-list li:hover {
      background-color: #e0f7fa; /* 鼠标悬停时的浅蓝色背景 */
      color: #2168BE; 
  }
  
  .active-selection {
      background-color: #2168BE !important; /* 选中时的蓝色背景 */
      color: white !important; /* 选中时的白色文字 */
      font-weight: bold;
  }
  
  .active-selection .b-icon {
