    ...
    <b-row v-if="loading || isRunning" class="justify-content-center mt-3">
        <b-col cols="10" class="text-center">
            <b-alert show variant="info" class="p-2">
                ...
                <span v-if="progress.progress_percentage > 0"> (进度: {{ progress.progress_percentage.toFixed(2) }}%)</span>
              ...
              <div class="module-content">
                <div class="top-row">
                    ...
                    <div class="deviation-box">
                      <p>偏差值:</p>
                      <span class="deviation-value" :class="getDeviationClass(module.deviation)">{{ module.deviation.toFixed(4) }}</span>
                    </div>
                </div>
            ...
            <div v-if="activeModule && activeModule.name === module.name" class="module-details-overlay p-4">
              ...
              <div class="detail-section mt-4">
                <div class="detail-item">
                    ...
                    <span class="deviation-value-large" :class="getDeviationClass(activeModule.deviation)">{{ activeModule.deviation.toFixed(4) }}</span>
                </div>
              </div>
      ...
      <b-col cols="5" class="text-center mx-3">
        ...
        <div class="info-box p-3">
          ...
          <div class="mt-3 text-right">
            ...
            <span class="deviation-value-bottom" :class="getDeviationClass(maxErrorDeviation)">{{ maxErrorDeviation.toFixed(4) }}</span>
          </div>
        </div>
      </b-col>
...
export default {
  ...
  data() {
    return {
      ...
      // API 状态 (修复：确保 loading 和 isRunning 在 data 中)
      loading: true,
      isRunning: false,
      ...
      // 诊断结果 (对应设计图底部)
      globalAccuracy: 0.0, // 对应设计图的“准确率”
      recallRate: 0.0, // 对应设计图的“不一致性根因召回率”
      maxErrorModule: 'N/A', // 对应设计图的“偏差模块”
    };
  },
  computed: {
    // ------------------------------------------------------------------
    // 模块偏差测试结果 (处理后的数据) - 修复：确保 processedModules 在 computed 中
    // ------------------------------------------------------------------
    processedModules() {
        // 将 rawModules 转换为适用于模板的列表
        return Object.keys(this.rawModules).map(key => {
            const mod = this.rawModules[key];
            const name = MODULE_NAMES[key] || key;
            
            // 从 single_task_stage 获取预测和真值
            const stage = mod.single_task_stage || {};
            const prediction = stage.prediction || {};
            const groundTruth = stage.ground_truth || {};
            
            // 从 analysis_task 获取偏差值 (calculated_value)
            const analysis = mod.analysis_task || {};
            const deviation = analysis.calculated_value || 0;

            // 简化输出和真值 (这里需要根据实际数据结构进一步简化)
    ...
    // ------------------------------------------------------------------
    // LLM 认知传播偏差测试结果 (总偏差值) - 修复：确保 maxErrorDeviation 在 computed 中
    // ------------------------------------------------------------------
    maxErrorDeviation() {
        // 确保返回一个数值，避免 undefined.toFixed() 错误
        return this.rawLlmAnalysis?.max_error || 0.0; 
    },
    
    // ------------------------------------------------------------------
    // LLM 认知传播偏差测试结果 (总览文本) - 修复：确保 llm_analysisSummaryText 在 computed 中
    // ------------------------------------------------------------------
    llm_analysisSummaryText() {
        // 简单汇总所有原因分析
        if (!this.rawLlmAnalysis) return '等待 LLM 分析结果...';
        
        // 修复：使用 computed 属性时确保它已定义
  ...
  methods: {
    ...
    async fetchStatus() {
        ...
        try {
            ...
            if (!this.isRunning && data.success) {
                // 假设 LLM 分析完成后，后端会把一些全局指标也放入 status 接口，
                // 或者我们从一个统一的 API (例如 /api/analyze-with-llm) 获取。
                // 暂时从 analyze-with-llm 接口获取全局指标
    ...
    async fetchAnalysisMetrics() {
        try {
            ...
            if (data) {
                this.globalAccuracy = data.precision || 0.0; // 假设 precision 对应准确率
                this.recallRate = data.recall || 0.0; // 对应召回率
                
                // 确定最大偏差模块（根据LLM返回或根据模块偏差值）
                if (this.rawLlmAnalysis && this.rawLlmAnalysis.max_error) {
                    // 从 LLM 分析中获取最大偏差的模块名 (例如 'module1' 对应 reason1)
                    // 由于后端只返回 max_error 的值，这里需要进一步逻辑来确定是哪个模块。
                    // 假设 max_error 对应 reasonX 中描述的模块。
                    let maxKey = null;
                    let maxVal = 0;
                    ...
                    this.maxErrorModule = maxKey || 'N/A';
                } else {
                    this.maxErrorModule = 'N/A';
                }
            }
        } catch (error) {
    ...
    getDeviationClass(deviation) {
      ...
      // 修复：确保在 deviation 为 null/undefined 时不报错
