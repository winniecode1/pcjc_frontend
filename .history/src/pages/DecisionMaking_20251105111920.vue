      ...
      setTimeout(() => {
        try {
          ...
          const mockBackendData = {
            status: "success",
            data: {
              weapon_model: "Su-35 (模拟)",
              summary: "这是模拟的本阶段产生的文本信息。分析完成，一切正常。",
              ...
              // "危险等级3" -> 3 !
              // "危险等级4" -> 4 !
              model_analysis_danger_level: "危险等级1",
              local_txt_danger_level: "危险等级1",
              
              // 图像列表 (模拟)
              image_urls: [
                '/path/to/mock/image1.png', // 您可以使用公共可访问的图片URL测试
                '/path/to/mock/image2.png',
                '/path/to/mock/image3.png',
                '/path/to/mock/image4.png'
              ]
            }
          };

          // --- 2. 检查模拟数据状态并解析 ---
          if (mockBackendData.status === 'success') {
            // 直接调用数据解析函数，和真实API一样
            this.parseBackendData(mockBackendData.data);
          } else {
            this.currentStageText = `模拟数据获取失败`;
          }

        } catch (error) {
          console.error('模拟数据解析失败：', error);
          this.currentStageText = '模拟数据解析失败';
        } finally {
