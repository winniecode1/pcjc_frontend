...
parseBackendData(backendData) {
    ...
    // 2. 本阶段文本（后端summary，保持不变）
    this.currentStageText = backendData.summary || '设备性能分析完成，危险等级已评估';
    
    // 3. 多时序图像 (!!! 关键修改：直接使用后端返回的 URL 列表 !!!)
    // 注意：这里的 image_urls 是我们在 fetchBackendData 中手动添加进去的
    this.imageList = backendData.image_urls || []; 
    if (this.imageList.length === 0) {
        console.warn("图片URL列表为空，请检查12358接口。");
    }

    // 4. 设备描述（使用 summary，保持不变）
    this.deviceDescription = backendData.summary || '模型分析结果未返回。'; 
