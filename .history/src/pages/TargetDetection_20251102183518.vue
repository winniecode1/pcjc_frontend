...
<style lang="scss" scoped>
  ...
  .selected-list {
      ...
      width: 100%;
      // 关键修改：移除 flex-grow: 1，明确设置高度
      height: calc(100% - 130px); /* 70vh - (顶部padding + 标题h5 + 按钮区域 + 底部p) 的估算值 */
      overflow: hidden; /* 隐藏容器溢出部分 */
      padding-left: 50px !important;
  }
  
  .video-list {
      // 关键修改：将 max-height 更改为明确的高度，并使用 !important 提高优先级
      height: 100% !important; 
      overflow-y: auto !important; /* 确保滚动条生效 */
      padding-right: 15px; 
  }
